...
# quelques lignes qui appellent les clés d'API pour le streaming
...
# client.update('Mon tweet de test!!!!')

require 'pry'


# client.follow('edtoulet')
# client.follow('bmdev4')
...
while journalist_tab.length < 5 do
    var = rand(0..journalist.length)
    journalist_tab << ((journalist[var]))
end

text =  journalist_tab.join(' ')

#client.update("un mort vivant est un oxymore alors qu'un occis mort est un pleonasme #{text} #bonjour_monde")
...
####### SEARCH et fav(like) et follow

# client.search("#bonjour_monde -rt").take(2).each do |tweet|
# puts tweet.text
# client.fav tweet
...
# https://www.rubydoc.info/gems/twitter/Twitter/REST/DirectMessages#create_direct_message-instance_method
